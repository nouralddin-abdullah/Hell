...
const StorePage = () => {
  ...
  const getSortParam = () => {
    switch (sortOrder) {
      case "lowest": return "price";
      case "highest": return "-price"; 
      default: return "";
    }
  };
  ...
  const handlePageChange = (page: number) => {
    const totalPages = activeTab === "Buy" 
    ...
    <section className="store-main">
      <div className="store-container">
        <div className="store-nav">
          <div className="coins-container">
            ...
            {isUserLoading ? (
              ...
              <p>{currentUser?.user?.stats?.coins ?? currentUser?.user?.points ?? 0}</p>
            )}
          </div>
          <div 
        ...
        <div className="frames-container">
          {isLoading ? (
            ...
            <div className="store-frames">
              {getCurrentItems().map((item) => (
                <div 
                  key={item.id} 
                  className={`frame-card ${isStoreItem(item) && !item.canAfford ? "not-enough-coins" : ""}`}
                  onMouseEnter={() => setHoveredCardId(item.id)}
                  onMouseLeave={() => setHoveredCardId(null)}
                >
                  <div className="image-and-frame">
                    <Avatar 
                      photo={currentUser?.user?.photo ? `${baseURL}/profilePics/${currentUser.user.photo}` : profileImage} 
                      userFrame={item.URL}
                  ...
                  {activeTab === "Buy" ? (
                    ...
                    <button 
                      ...
                      className={item.id === equippedFrameData?.data?.id ? 'equipped' : 'equip-button'}
                      disabled={item.id === equippedFrameData?.data?.id}
                    >
                      {item.id === equippedFrameData?.data?.id ? "Equipped" : "Equip"}
                    </button>
                  )}
                </div>
              ))}
          ...
          {/* Pagination */}
          {!isLoading && getCurrentPagination() && (getCurrentPagination()?.totalPages ?? 0) > 1 && (
            <div className="page-pagination">
              ...
              {Array.from({ length: Math.min(5, getCurrentPagination()?.totalPages ?? 0) }, (_, i) => {
                let pageNum;
                const totalPages = getCurrentPagination()?.totalPages ?? 0;
                if (totalPages <= 5) {
                  pageNum = i + 1;
              ...
              {(getCurrentPagination()?.totalPages ?? 0) > 5 && currentPage < (getCurrentPagination()?.totalPages ?? 0) - 2 && (
                <>
                  <span>...</span>
                  <a
                    href="#"
              ...
              <FontAwesomeIcon
                ...
                onClick={() => handlePageChange(currentPage + 1)}
                style={{ 
                  cursor: currentPage < (getCurrentPagination()?.totalPages ?? 0) ? "pointer" : "default", 
                  opacity: currentPage < (getCurrentPagination()?.totalPages ?? 0) ? 1 : 0.5 
                }}
              />
            </div>
          )}
      ...
      <Modal isOpen={isPreviewModalOpen} onClose={() => setIsPreviewModalOpen(false)}>
        {previewItem && (
          <div style={{ padding: "1rem", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-around", gap: "1rem", marginTop: "2rem" }}>
              <Button 
              ...
              <Button 
                ...
                style={!previewItem.canAfford ? { backgroundColor: "#ccc", cursor: "not-allowed" } : undefined}
              >
            ...
            return (
              <p style={{ marginBottom: "2rem" }}>
                ...
                {item && isStoreItem(item) && (
                  <span style={{ display: "block", fontWeight: "bold", marginTop: "0.5rem" }}>
